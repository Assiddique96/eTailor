...
export function NotificationsBell() {
  ...
  function getNotificationUrl(n: { entityType?: string; entityId?: string }): string | null {
    if (!n.entityId || !n.entityType) return null;
    switch (n.entityType) {
      case "Job":      return `/jobs/${n.entityId}`;
      case "Customer": return `/customers/${n.entityId}`;
      case "Invoice":  return `/billing/${n.entityId}`;
  ...
  function handleToggle() {
    if (!open && buttonRef.current) {
      ...
      const dropdownHeight = 350; // max-h-80 (320px) + header (~50px)
                    ...
                    <div className="flex flex-col items-end gap-1 flex-shrink-0">
                      ...
                      {url && (
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                          stroke="currentColor" strokeWidth="2" className="text-muted mt-1" aria-hidden>
