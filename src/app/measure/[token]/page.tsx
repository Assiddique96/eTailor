...
export default function RemoteMeasurementPage({
  ...
  useEffect(() => {
    fetch(`/api/measurement-links/${token}`)
      .then(async (res) => {
        ...
        if (!res.ok) { setErrorMsg(data.error ?? "Invalid link."); setState("error"); return; }
        setCtx(data);
        setState("ready");
      })
      .catch(() => { setErrorMsg("Could not load this link. Please try again."); setState("error"); });
  }, [token]);
  ...
  const Shell = ({ children }: { children: React.ReactNode }) => (
    <div style={{
      minHeight: "100vh", background: "var(--bg-base, #f7f6f3)",
      ...
      <div style={{ width: "100%", maxWidth: 660 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          {ctx && (
            <p style={{ margin: 0, color: "var(--text-muted, #57534e)", fontSize: 14 }}>
              {ctx.shopName}
            </p>
          )}
        </div>
  ...
  if (state === "error") return (
    <Shell>
      <div className="card p-8 text-center">
        ...
        <h2 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 600 }}>Link unavailable</h2>
        <p style={{ color: "var(--text-secondary)", fontSize: 14, margin: 0 }}>{errorMsg}</p>
      </div>
    </Shell>
  );
    ...
    <Shell>
      <div className="card p-8 text-center">
        ...
        <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.6 }}>
          Thank you, <strong>{ctx?.customer.firstName}</strong>. Your tailor at{" "}
          <strong>{ctx?.shopName}</strong> will use these to prepare your garments.
  ...
  return (
    <Shell>
      <div className="card" style={{ overflow: "hidden" }}>
        {/* Customer greeting */}
        <div style={{
          padding: "20px 24px", borderBottom: "1px solid var(--border, #e7e5e4)",
          ...
          <p style={{ margin: 0, color: "var(--text-secondary)", fontSize: 13 }}>
            {ctx!.gender === "MALE" ? "Men's" : ctx!.gender === "FEMALE" ? "Women's" : "General"}{" "}
        ...
        <div style={{ padding: 24 }}>
          <p style={{
            ...
            Fill in your measurements in centimetres (cm). Use a soft measuring tape and ask
            a friend to help for more accurate results. The diagram updates as you fill in each field.
          </p>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                {customFields.length === 0 ? (
                  <p style={{ fontSize: 12, color: "var(--text-muted)" }}>
                    If your tailor asked for a specific measurement not listed above
                    (e.g. "Thigh", "Ankle", "Back length"), you can add it here.
                  </p>
                ) : (
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <p style={{
              ...
              🔒 This link is single-use and private. Your data goes directly to your tailor.
            </p>
          </div>
        </div>
      </div>
